...
if [ -z "$KF5_SNAP_WRAPPING" ]; then
  SNAP=/snap/kf5-5-106-qt-5-15-9-core22-sdk/current
...
# Set suitable qtchooser configs (the config dir is set
# by the wrapper),
...
FileUtils.mkpath("/usr/lib/#{gnu_arch}-linux-gnu/qt-default/qtchooser", verbose: true)
File.write("/usr/lib/#{gnu_arch}-linux-gnu/qt-default/qtchooser/default.conf", <<-CONF)
/usr/lib/qt5/bin
/usr/lib/#{gnu_arch}-linux-gnu
CONF
FileUtils.ln_s('default.conf', "/usr/lib/#{gnu_arch}-linux-gnu/qt-default/qtchooser/qt5.conf", force: true)
FileUtils.ln_s('default.conf', "/usr/lib/#{gnu_arch}-linux-gnu/qt-default/qtchooser/5.conf", force: true)
